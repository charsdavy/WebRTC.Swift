...
//  Herlo
//
//  Created by dengw on 2022/8/8.
//

// WebRTC Version 104.0.0
...
final class WebRTCClient: NSObject {
    ...
    // MARK: - Media
    func startCaptureLocalVideo(renderer: RTCVideoRenderer, position: AVCaptureDevice.Position = .front) {
        guard let capturer = self.videoCapturer as? RTCCameraVideoCapturer else {
            return
        }

        guard
            let camera = (RTCCameraVideoCapturer.captureDevices().first { $0.position == position }),
